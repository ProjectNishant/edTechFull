<template>
    <div>
        ...
        <div id="haventBuy"><h2>You haven't buy any use case till now.</h2></div>
        <div id="unAuth" style="display:none;">
          <h2>You have to login first.</h2>
          <button class="btn btn-secondary">
            <sidebar-link to="/stats" name="Login Now"/>
          </button>
        </div>
        <div id="freeUseCase">
    <div class="row">
    ...
    <div class="card position-relative shadow-lg bg-white programCard">
        ...
        <div class="card-body pt-5">
          ...
          <div class="float-right pt-5 pb-2" v-if="useCase.price">
            <span type="hollow" class=" dHfCy3" v-on:click="gotoData(useCase._id, useCase.redirectLink, useCase.price)">Enroll <i class="fa fa-sign-in"></i></span>
          </div>
          <div class="float-right pt-5 pb-2" v-if="!(useCase.price)">
    ...
    export default {
        ...
        methods: { 
          loadCards: function(){
            ...
            if(authTok == null || authTok == undefined){
                document.getElementById('unAuth').style.display = 'block';
                document.getElementById('haventBuy').style.display = 'none';
              ...
              if(authTok != undefined){
                axios({
                    method: 'GET',
                    url: 'https://cors-anywhere.herokuapp.com/https://boiling-garden-13643.herokuapp.com/posts/getEnrolledUseCase',
                    headers:{authorization : authTok},
                    validateStatus: () => true
                }).then(response => {(this.fullData=response.data.useCaseDetails)
                // console.log(authTok, this.fullData)
                document.getElementById('load').style.display = 'none';
                document.getElementById('haventBuy').style.display = 'none';
                });
